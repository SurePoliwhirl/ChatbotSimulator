...
목표:
- 위 주제 상황에서
- 페르소나1과 페르소나2가 멀티턴 대화를 자연스럽게 이어갈 수 있도록
- 각자가 사용할 “역할 프롬프트”를 생성한다.

중요한 전제:
- system_prompt를 만들지 마라.
- 하나의 공통 규칙을 만들지 마라.
...
[3] “하지 말아야 할 행동”을 반드시 포함하라.
- 이 역할이 하면 어색해지는 행동을 명시적으로 금지하라.
- 예: 안내 금지, 설명 금지, 결정 금지, 책임 전가 금지 등
- 특히 미팅 제안, 일정 조율, 잡담, 관계 형성으로 흐르는 행동을 금지하라.
...
- 질문 → 설명 → 비교 → 판단 흐름이 자연스럽게 이어지도록 설계하라.
